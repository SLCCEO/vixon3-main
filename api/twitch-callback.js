...
export default async function handler(request, response) {
  ...
  if (error) return response.status(400).send(`Twitch authorization failed: ${errorDescription || error}`);
  if (!code) return response.status(400).send('Missing Twitch authorization code. Start at https://www.vixon.online/api/twitch-login.');
  if (!process.env.TWITCH_CLIENT_ID || !process.env.TWITCH_CLIENT_SECRET) return response.status(503).send('Twitch API credentials are not configured on Vercel.');
